...
class AccountStatementReportPage extends StatefulWidget {
  ...
  @override
  State<AccountStatementReportPage> createState() => _AccountStatementReportPageState();
}

class _AccountStatementReportPageState extends State<AccountStatementReportPage> {
  ...
  Widget _buildAccountSelector(BuildContext context, AccountStatementState state) {
    if (state is AccountStatementLoaded) {
      return Container(
        ...
        child: DropdownButtonFormField<int>(
          ...
          items: state.accounts.map((a) => DropdownMenuItem(
            value: a['id'] as int,
            child: Text('${a['code']} - ${a['name']}', style: const TextStyle(fontSize: 14)),
          )).toList(),
          onChanged: (v) => v != null ? context.read<AccountStatementCubit>().selectAccount(v) : null,
        ),
      );
    }
  ...
  Future<void> _exportPdf(BuildContext context) async {
    ...
    await ExportService.printData(
      title: 'كشف حساب: ${_lastState!.accounts.firstWhere((a) => a['id'] == _lastState!.selectedAccountId)['name']}',
      headers: headers,
      data: data,
      settings: settings,
    );
...
class _AccountStatementContentState extends State<_AccountStatementContent> {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AccountStatementCubit, AccountStatementState>(
      builder: (context, state) {
        if (state is AccountStatementLoading) return const Center(child: CircularProgressIndicator());
        if (state is AccountStatementError) return Center(child: Text('خطأ: ${state.message}', style: const TextStyle(color: Colors.red)));
        if (state is AccountStatementLoaded) {
          if (state.transactions.isEmpty) {
            return Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.receipt_long, size: 48, color: Colors.grey[300]), const SizedBox(height: 16), const Text('لا توجد حركات مسجلة للحساب المختار', style: TextStyle(color: Colors.grey))]));
          }
          return Column(
            children: [
              _buildSummaryArea(state.summary),
              Expanded(
                child: ListView.builder(
                  padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                  itemCount: state.transactions.length,
                  itemBuilder: (context, index) => _buildTransactionCard(state.transactions[index]),
                ),
              ),
            ],
          );
  ...
  Widget _buildSummaryArea(dynamic summary) {
    return Container(
      ...
      child: ListView(
        ...
        children: [
          _buildSummaryItem('رصيد افتتاحي', summary.openingBalance, Colors.blue),
          _buildSummaryItem('إجمالي مديونية', summary.totalDebits, Colors.green),
          _buildSummaryItem('إجمالي دائنية', summary.totalCredits, Colors.red),
          _buildSummaryItem('الرصيد النهائي', summary.closingBalance, Colors.deepPurple, isBold: true),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          FittedBox(child: Text('${_numberFormat.format(value)} ر.س', style: TextStyle(color: color, fontSize: 13, fontWeight: isBold ? FontWeight.bold : FontWeight.w500))),
        ],
      ),
    );
  }
      ...
      child: Column(
        children: [
          ListTile(
            title: Row(
              ...
              children: [
                ...
                Text('${_numberFormat.format(t.balance)} ر.س', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Colors.blueGrey)),
              ],
            ),
            subtitle: Padding(
              ...
              child: Row(
                children: [
                  ...
                  Text('${t.transactionDate.day}/${t.transactionDate.month}/${t.transactionDate.year}', style: const TextStyle(fontSize: 11)),
                  const SizedBox(width: 16),
                  const Icon(Icons.tag, size: 12, color: Colors.grey),
                  const SizedBox(width: 4),
                  Expanded(child: Text(t.reference, style: const TextStyle(fontSize: 11), overflow: TextOverflow.ellipsis)),
                ],
              ),
            ),
          ),
          Padding(
            ...
            child: Row(
              children: [
                 _buildAmountBadge('مدين: ${_numberFormat.format(t.debitAmount)}', Colors.green),
                 const SizedBox(width: 10),
                 _buildAmountBadge('دائن: ${_numberFormat.format(t.creditAmount)}', Colors.red),
              ],
            ),
          ),
        ],
