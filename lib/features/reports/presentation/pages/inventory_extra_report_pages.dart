      ...
      builder: (context, snapshot) {
        ...
        return Column(children: [
          ...
          Expanded(child: ListView.builder(padding: const EdgeInsets.all(16), itemCount: data.rows.length, itemBuilder: (context, index) {
            ...
            return Card(elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppRadius.lg), side: BorderSide(color: Colors.grey[100]!)), margin: const EdgeInsets.only(bottom: 12), child: ListTile(title: Text(r.productName, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)), subtitle: Text('${r.dateLabel} | مرجع: ${r.referenceNo}', style: const TextStyle(fontSize: 10)), trailing: Text(r.qtyIn > 0 ? '+${r.qtyIn.toInt()}' : '-${r.qtyOut.toInt()}', style: TextStyle(fontWeight: FontWeight.bold, color: r.qtyIn > 0 ? Colors.green : Colors.red, fontSize: 16))));
          })),
        ]);
      },
    );
    ...
    return FutureBuilder<List<_LowStockRow>>(
      ...
      builder: (context, snapshot) {
        ...
        return ListView.builder(padding: const EdgeInsets.all(16), itemCount: rows.length, itemBuilder: (context, index) {
          final r = rows[index];
          return Card(elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppRadius.lg), side: const BorderSide(color: AppColors.orange500Alpha)), margin: const EdgeInsets.only(bottom: 12), child: ListTile(title: Text(r.name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)), subtitle: Text('حد الطلب: ${r.min}', style: const TextStyle(fontSize: 10)), trailing: Text('${r.qty}', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.orange, fontSize: 16))));
        });
      },
    );
  }
      ...
      builder: (context, snapshot) {
        ...
        return Column(children: [
          ...
          Expanded(child: ListView.builder(padding: const EdgeInsets.all(16), itemCount: rows.length, itemBuilder: (context, index) {
            final r = rows[index];
            return Card(elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(AppRadius.lg), side: BorderSide(color: Colors.grey[100]!)), margin: const EdgeInsets.only(bottom: 12), child: ListTile(title: Text(r.name, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 13)), subtitle: Text('الكمية: ${r.qty}', style: const TextStyle(fontSize: 10)), trailing: Text('${fmt.format(r.value)} ر.س', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.purple, fontSize: 14))));
          })),
        ]);
      },
    );
