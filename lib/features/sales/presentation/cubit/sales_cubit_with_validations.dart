...
class SalesInvoicePageExample extends StatelessWidget {
  const SalesInvoicePageExample({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => SalesCubitWithValidations(
        accountLimitService: context.read<AccountLimitService>(),
        accountConnectValidator: context.read<AccountConnectValidator>(),
        initialState: SalesInitial(),
      )..checkSystemReadiness(context), // Check on startup
      child: Scaffold(
        appBar: AppBar(title: const Text('فاتورة مبيعات')),
        body: BlocBuilder<SalesCubitWithValidations, dynamic>(
          builder: (context, state) {
            return Center(
              child: ElevatedButton(
                onPressed: () async {
                  final cubit = context.read<SalesCubitWithValidations>();
                  
                  // Validate and save invoice
                  final success = await cubit.validateAndSaveInvoice(
...
// Dummy state classes for example
